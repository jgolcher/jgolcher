...
\CommentTok{#Part 1}
\CommentTok{#Note how the base R grammar we learned earlier differs a lot from "ggplot2" and "dplyr" (a series of packages as part of the "Tidyverse") we will use now to visualize the same data:}
...
\NormalTok{only_1class <-}\StringTok{ }\KeywordTok{filter}\NormalTok{(titanic, pclass }\OperatorTok{==}\StringTok{ "1"}\NormalTok{) }\CommentTok{#Extract rows that match a given criteria (like subset() in base R)}
\NormalTok{countstitanic<-}\StringTok{ }\NormalTok{titanic }\OperatorTok{%>%}\StringTok{ }\KeywordTok{count}\NormalTok{(sex,pclass,age, }\DataTypeTok{sort =} \OtherTok{TRUE}\NormalTok{)}
\NormalTok{widetitanic <-}\StringTok{ }\NormalTok{countstitanic }\OperatorTok{%>%}\StringTok{ }\KeywordTok{spread}\NormalTok{(sex,n) }\CommentTok{#Sometimes we want to have a "wide" format}
\NormalTok{longtitanic <-}\StringTok{ }\NormalTok{widetitanic }\OperatorTok{%>%}\StringTok{ }\KeywordTok{gather}\NormalTok{( sex, n, female}\OperatorTok{:}\NormalTok{male, }\DataTypeTok{factor_key=}\OtherTok{TRUE}\NormalTok{)}\CommentTok{#this can reverse back to "long"}
\NormalTok{titanic }\OperatorTok{%>%}\StringTok{ }\KeywordTok{describe_tbl}\NormalTok{() }\CommentTok{#Describes data}
\NormalTok{summary_titanic<-}\StringTok{ }\NormalTok{titanic }\OperatorTok{%>%}\KeywordTok{group_by}\NormalTok{(sex, pclass) }\OperatorTok{%>%}\NormalTok{dplyr}\OperatorTok{::}\KeywordTok{summarize}\NormalTok{(}\DataTypeTok{Mean =} \KeywordTok{mean}\NormalTok{(age, }\DataTypeTok{na.rm=}\OtherTok{TRUE}\NormalTok{))}\CommentTok{#Generate summary}
...
\NormalTok{p1}
\CommentTok{#Packages like "dplyr" and "ggplot2" do a lot of the hard initial coding/data wrangling for you, giving you more time to focus on more advanced functions to use with your data.}
\end{Highlighting}
\end{Shaded}
